...
import { generateVerdict, generateRoast, NO_CODE_VERDICT, NO_CODE_ROAST } from '../utils/verdict';
...
export async function analyzeToken(
  ...
  try {
    ...
    if (!rawBytecode || rawBytecode === '0x') {
      return {
        ...
        data: {
          ...
          narrative_verdict: NO_CODE_VERDICT,
          roast: NO_CODE_ROAST,
    ...
    const results = compareToLibrary(resolved.bytecode, getProcessedLibrary());
    ...
    const analysisResult = {
      ...
      is_proxy: resolved.is_proxy,
      proxy_implementation: resolved.implementation_address,
      bytecode_size: (resolved.bytecode.length - 2) / 2,
      analysis_time_ms: Date.now() - startTime,
    };
